...
#pragma warning restore CS1998
        {
            ...
            // All applications must have an enabled ARFoundation ARAnchorManager and ARSessionOrigin
        ...
        protected virtual void OnAnchorLocated(object sender, AnchorLocatedEventArgs args)
        {
            ...
            if (args.Status == LocateAnchorStatus.Located)
            {
                ...
                UnityEngine.Quaternion anchorToUnityWorldRotation = Quaternion.identity;
                UnityEngine.Vector3 anchorToUnityWorldTranslation = UnityEngine.Vector3.zero;
#if ENABLE_WINMD_SUPPORT
...
#else
                SpatialCoordinateSystem unityWorldCoordinateSystem = Marshal.GetObjectForIUnknown(WindowsMREnvironment.OriginSpatialCoordinateSystem) as SpatialCoordinateSystem;
#endif
                ...
                if (anchorToUnityWorld.HasValue)
                {
                    ...
                    anchorToUnityWorldRotation = ToUnityQuaternion(anchorToUnityWorldRotationNative);
                    anchorToUnityWorldTranslation = ToUnityVector3(anchorToUnityWorld.Value.Translation);
                } 
                else 
                {
                ...
                Pose anchorToUnityWorldPose = new Pose(anchorToUnityWorldTranslation, anchorToUnityWorldRotation);
                UnityDispatcher.InvokeOnAppThread(async () =>
                    {
                        ARAnchor worldAnchor = SpatialAnchorManager.arAnchorManager.AddAnchor(anchorToUnityWorldPose);

                        // Wait up to one second until AR Foundation creates a backing platform's anchor which normally takes about one frame
                        int retryCount = 30;
                        while (worldAnchor.nativePtr == IntPtr.Zero && retryCount > 0)
                        {
                            await Task.Delay(33); // in milliseconds
                            --retryCount;
                        }

                        if (worldAnchor.nativePtr == IntPtr.Zero)
                        {
                            throw new InvalidOperationException("Couldn't obtain a native anchor pointer");
                        }

#if MIXED_REALITY_OPENXR
                        // wait for MR OpenXR to return a valid platform pointer
                        retryCount = 30;
                        while (worldAnchor.nativePtr.GetPlatformPointer() == IntPtr.Zero && retryCount > 0)
                        {
                            await Task.Delay(33); // in milliseconds
                            --retryCount;
                        }

                        if (worldAnchor.nativePtr.GetPlatformPointer() == IntPtr.Zero)
                        {
                            throw new InvalidOperationException("Couldn't obtain a platform pointer");
                        }
#endif // MIXED_REALITY_OPENXR

                        // For WMR, replace the native anchor pointer here so that we don't leak the ARAnchor
                        // We cannot use the original native anchor since we won't get notified when the anchor is destroyed
                        args.Anchor.LocalAnchor = worldAnchor.nativePtr.GetPlatformPointer();

                        // Now that the platform pointer is valid, that means that anchorsChanged() has already
                        // fired and pointerToAnchors has been updated, so we can immediately fire the AnchorLocated event
                        AnchorLocated?.Invoke(this, args);
                    });
#else // WINDOWS_UWP || UNITY_WSA
                lock (pendingEventArgs)
                {
...
#region Event Handlers
        ...
        /// added/updated/removed by AR Foundation.  This is critical for mapping Azure Spatial
        /// Anchors CloudSpatialAnchors to Unity AR Foundation ARAnchors. Note: When you destroy an
        /// ARAnchor, ARFoundation will not fire an anchorsChanged event for it, so some of these
        /// anchors are already cleaned-up everywhere except in pointerToAnchors.
        /// </summary>
        ...
        private void ARAnchorManager_anchorsChanged(ARAnchorsChangedEventArgs obj)
        {
            lock (pointerToAnchors)
            {
                foreach (ARAnchor addedAnchor in obj.added)
                {
                    ...
                    // We allow an update here because ARAnchorManager will re-use addresses and
                    // the old anchor (with the same address/key) may not have been properly
                    // removed from pointerToAnchors. We cannot remove the old entry before this
                    // because the new pointer/key is only available around the same time as the
                    // anchorsChanged event is fired.
                    pointerToAnchors[lookupKey] = addedAnchor;
                }
